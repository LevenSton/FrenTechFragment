/**
 *Submitted for verification at lineascan.build on 2023-09-27
 */
...
contract Tomo is Ownable, ReentrancyGuard {
    ...
    bytes32 public constant BIND_TYPEHASH =
        keccak256(abi.encodePacked("Bind(bytes32 subject,address owner)"));
    bytes32 public constant REWARD_TYPEHASH =
        keccak256(
            abi.encodePacked(
                "Reward(bytes32 subject,address owner,uint256 snapshotReward,uint256 rewardPercent,bytes32 merkleRoot)"
            )
        );
    ...
    function claimReward(
        ...
        require(
            MerkleProof.verify(merkleProof, vp.merkleRoot, node),
            "Invalid proof"
        );

        uint256 claimedWordIndex = claimedIndex / 256;
        uint256 claimedBitIndex = claimedIndex % 256;
        vp.claimedBitMap[claimedWordIndex] =
            vp.claimedBitMap[claimedWordIndex] |
            (1 << claimedBitIndex);
    ...
    function setSubjectReward(
        ...
        emit SubjectRewardSet(
            SubjectRewardSetEvent({
    ...
    function getSubjectTotalReward(
        bytes32 subject
    ) public view returns (uint256) {
        return votePasses[subject].totalReward;
    }

    function getSubjectCurrentReward(
        bytes32 subject
    ) public view returns (uint256) {
        return votePasses[subject].currentReward;
    }

    function getSubjectMerkleRoot(
        bytes32 subject
    ...
    function isClaimed(
        ...
        uint256 claimedWord = votePasses[subject].claimedBitMap[
            claimedWordIndex
        ];
